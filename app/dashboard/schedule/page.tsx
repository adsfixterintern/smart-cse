            ...
            <div className="max-w-2xl">
              <div className="flex flex-wrap items-center gap-3 mb-4">
                ...
                <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 whitespace-nowrap">
                  <Clock className="w-3 h-3" /> Updated: {studentRoutine?.updatedAt ? new Date(studentRoutine.updatedAt).toLocaleDateString() : "Recently"}
                </span>
              </div>
              ...
              <p className="text-slate-400 font-medium italic mt-4 text-sm md:text-base">
                Showing routine for <span className="text-white underline">{user?.semester || "N/A"}th Semester</span>
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <Card className="lg:col-span-8 border-none shadow-2xl shadow-blue-500/5 rounded-[2rem] bg-white overflow-hidden">
          <CardHeader className="p-6 md:p-8 border-b border-slate-50 flex flex-row items-center justify-between">
            <div className="overflow-hidden">
              <CardTitle className="italic font-black uppercase text-lg md:text-xl text-slate-800 truncate">Visual Schedule</CardTitle>
              <CardDescription className="italic font-medium text-xs md:text-sm">Click to expand</CardDescription>
            </div>
          ...
          <CardContent className="p-4 md:p-6 bg-slate-50/50">
            {studentRoutine ? (
              <div className="relative group cursor-zoom-in rounded-xl md:rounded-[1.5rem] overflow-hidden border-2 md:border-4 border-white shadow-md">
                ...
                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4">
                  <Button 
                    ...
                    onClick={() => window.open(studentRoutine.imageUrl, '_blank')}
                  >
                    <ExternalLink className="w-3 h-3 mr-2" /> View Full Resolution
                  </Button>
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center py-20 text-center px-4">
                ...
                <h3 className="text-lg font-black italic uppercase text-slate-400">No Routine Found</h3>
                <p className="text-xs text-slate-400 italic mt-1">Contact admin for {user?.semester}th semester schedule.</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="border-none shadow-xl rounded-[1.5rem] md:rounded-[2rem] bg-blue-600 text-white p-6 md:p-8">
            <h4 className="font-black italic uppercase text-[10px] mb-6 opacity-80 tracking-[0.2em]">Quick Info</h4>
            <div className="space-y-5">
              <div className="flex justify-between items-center border-b border-white/10 pb-3 gap-2">
                <span className="text-[10px] font-bold italic uppercase whitespace-nowrap">Semester</span>
                <span className="font-black italic text-sm truncate">{user?.semester || "N/A"}th</span>
              </div>
              <div className="flex justify-between items-center border-b border-white/10 pb-3 gap-2">
                <span className="text-[10px] font-bold italic uppercase whitespace-nowrap">Status</span>
                <Badge className="bg-emerald-400 text-slate-900 font-black italic text-[8px] px-2 py-0 border-none">Active</Badge>
              </div>
              <div className="flex justify-between items-center gap-2">
                <span className="text-[10px] font-bold italic uppercase whitespace-nowrap">Format</span>
                <span className="font-black italic uppercase text-[10px] text-blue-100 truncate">PNG / JPEG</span>
              </div>
            </div>
          </Card>
          ...
          <Card className="border-none shadow-xl rounded-[1.5rem] md:rounded-[2rem] bg-white p-6 md:p-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="font-black italic uppercase text-slate-400 text-[10px] tracking-[0.2em]">Important</h4>
            </div>
            <p className="text-[11px] font-medium italic text-slate-500 leading-relaxed break-words">
              Please keep this schedule saved. For any discrepancies in class timing, consult your department head immediately.
            </p>
          </Card>
        </div>
      </div>
