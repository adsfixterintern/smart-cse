...
export default function ResultsPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [transcriptRes, resultsRes] = await Promise.all([
          fetch(`${apiUrl}/my-transcript`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${apiUrl}/my-results`, { headers: { Authorization: `Bearer ${token}` } })
        ])
  ...
  const semesters = Array.from(new Set(results.map(r => r.semester))).sort((a, b) => Number(a) - Number(b))
    ...
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400 font-bold italic text-[10px] uppercase tracking-widest mt-1">Archive Search & Filter</p>
            </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="border-none shadow-xl rounded-[2rem] bg-slate-900 text-white p-8 relative overflow-hidden">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Cumulative GPA</p>
          <h2 className="text-5xl font-black italic mt-2 tracking-tighter">{transcript?.cgpa?.toFixed(2) || "0.00"}</h2>
        </Card>
        {/* Simplified other cards for brevity */}
        <Card className="border-none shadow-xl rounded-[2rem] bg-white p-8 border border-slate-50">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Showing</p>
            <h2 className="text-4xl font-black italic mt-2 text-blue-600">{filteredResults.length} Subjects</h2>
        </Card>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-none">
                <TableHead className="font-black italic uppercase text-[10px] text-slate-500 p-6">Subject Details</TableHead>
                <TableHead className="font-black italic uppercase text-[10px] text-slate-500 text-center">Semester</TableHead>
                <TableHead className="font-black italic uppercase text-[10px] text-slate-500 text-center">Grade</TableHead>
                <TableHead className="font-black italic uppercase text-[10px] text-slate-500 text-center">Point</TableHead>
                <TableHead className="font-black italic uppercase text-[10px] text-slate-500 text-right pr-10">Details</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredResults.length > 0 ? (
                filteredResults.map((res) => (
                  <TableRow key={res._id} className="hover:bg-slate-50/50 transition-colors border-slate-50">
                    <TableCell className="p-6">
                      <div className="font-black italic text-slate-800 uppercase leading-none text-base">{res.courseName}</div>
                      <div className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest flex items-center gap-2">
                        <span className="bg-slate-100 px-2 py-0.5 rounded text-blue-600">{res.courseCode}</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge variant="outline" className="rounded-lg font-black italic border-slate-200">SEM {res.semester}</Badge>
                    </TableCell>
                    <TableCell className="text-center font-black italic text-xl text-slate-800">{res.grade}</TableCell>
                    <TableCell className="text-center font-black italic text-blue-600">{res.point.toFixed(2)}</TableCell>
                    <TableCell className="text-right pr-6">
                      <Sheet>
                        <SheetTrigger asChild>
                          <Button size="icon" variant="ghost" className="rounded-xl hover:bg-blue-50 text-blue-600">
                        ...
                        <SheetContent className="w-full sm:max-w-md rounded-l-[3rem] border-none p-10 overflow-y-auto">
                            <SheetHeader className="space-y-4">
                                ...
                                <SheetTitle className="text-3xl font-black italic uppercase tracking-tighter text-slate-900 leading-none">
                                    Mark <span className="text-blue-600">Breakdown</span>
                                </SheetTitle>
                            ...
                            <div className="mt-12 space-y-8">
                                ...
                                {[
                                    { label: "Class Test", val: res.breakdown.ct, max: 10, color: "bg-blue-500" },
                                    { label: "Midterm Exam", val: res.breakdown.mid, max: 20, color: "bg-indigo-500" },
                                    ...
                                    { label: "Assignment", val: res.breakdown.assignment, max: 5, color: "bg-orange-500" },
                                    { label: "Presentation", val: res.breakdown.presentation, max: 5, color: "bg-purple-500" },
                                ].map((item) => (
                                    <div key={item.label} className="space-y-3">
                                        <div className="flex justify-between items-end">
                                            <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">{item.label}</span>
                                            <span className="text-sm font-black italic text-slate-900">{item.val} / {item.max}</span>
                                        </div>
                ...
                <TableRow>
                  <TableCell colSpan={5} className="p-20 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-20">
                      ...
                      <p className="font-black italic uppercase">No matching courses found</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
