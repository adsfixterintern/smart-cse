...
import { 
  Loader2, GraduationCap, Eye, Download, 
  BookOpen, FileImage, Calendar, Search 
} from "lucide-react"
import { 
  ...
  if (loading) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm font-medium text-muted-foreground italic uppercase tracking-widest">Fetching your curriculum...</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto space-y-8 p-4 md:p-8 animate-in fade-in duration-700">
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow className="hover:bg-transparent border-b">
                <TableHead className="w-[120px] font-black uppercase text-[10px] tracking-widest px-8">Code</TableHead>
                <TableHead className="font-black uppercase text-[10px] tracking-widest">Course Title</TableHead>
                <TableHead className="font-black uppercase text-[10px] tracking-widest hidden md:table-cell">Instructor</TableHead>
                <TableHead className="font-black uppercase text-[10px] tracking-widest text-center">Resources</TableHead>
            ...
            <TableBody>
              {courses.map((course) => (
                <TableRow key={course._id} className="group hover:bg-slate-50/50 transition-colors">
                  <TableCell className="px-8 font-bold text-primary italic">{course.code}</TableCell>
                  <TableCell>
                    <p className="font-black text-slate-800 uppercase tracking-tight">{course.name}</p>
                    <p className="text-[10px] text-slate-400 font-bold md:hidden">Instructor: {course.teacherName}</p>
                  </TableCell>
                  <TableCell className="hidden md:table-cell font-bold text-slate-600 italic">
                    {course.teacherName || "Not Assigned"}
                  </TableCell>
                  <TableCell className="text-center">
                    <Badge variant="secondary" className="bg-blue-50 text-blue-600 rounded-lg font-black">
      ...
      {courses.length === 0 && (
        <div className="py-20 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed">
          ...
          <p className="font-black text-slate-400 uppercase italic">No courses found for this semester</p>
        </div>
      )}
          ...
          <div className="p-6 space-y-4">
            {selectedCourse?.resources?.length > 0 ? (
              selectedCourse.resources.map((res: any, idx: number) => (
                <div key={idx} className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border group hover:border-primary transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black text-slate-800 truncate uppercase italic">{res.title}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase">Image Resource</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="icon" variant="ghost" className="rounded-full hover:bg-blue-100 text-blue-600" onClick={() => setPreviewImage(res.url)}>
                      <Eye size={18} />
                    </Button>
                    <Button size="icon" variant="ghost" className="rounded-full hover:bg-green-100 text-green-600" asChild>
              ...
              <div className="text-center py-12">
                <p className="text-slate-400 font-bold italic italic">No resources uploaded yet.</p>
              </div>
            )}
          </div>
        </SheetContent>
      ...
      <Dialog open={!!previewImage} onOpenChange={() => setPreviewImage(null)}>
        <DialogContent className="max-w-4xl p-0 overflow-hidden bg-black/90 border-none">
          ...
          <div className="absolute bottom-4 right-4">
            <Button onClick={() => setPreviewImage(null)} className="rounded-xl font-black italic">CLOSE PREVIEW</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
