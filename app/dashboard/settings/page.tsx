...
import { 
  User, Mail, Phone, Camera, Loader2, Save, 
  BadgeCheck, GraduationCap, BookOpen, Heart, Shield, Layout 
} from "lucide-react"
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const { _id, email, password, role, createdAt, ...cleanData } = formData;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12 animate-in fade-in duration-700">
      <div className="flex flex-col gap-1 px-4">
        ...
        <p className="text-slate-500 font-medium">Update and manage your university information.</p>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-[#0f172a] p-8 rounded-[2.5rem] text-white shadow-2xl flex flex-col items-center sticky top-6">
            ...
            <h2 className="text-xl font-bold flex items-center gap-2">
              {formData?.name} <BadgeCheck size={18} className="text-blue-500" />
            </h2>
            <p className="text-blue-400 font-mono text-sm mt-1">{formData?.studentId}</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input value={formData.name || ""} onChange={(e) => setFormData({...formData, name: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input value={formData.email || ""} disabled className="pl-10 h-12 bg-slate-50" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input value={formData.phone || ""} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input value={formData.batch || ""} onChange={(e) => setFormData({...formData, batch: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Current Semester</label>
                <div className="relative">
                  <BookOpen className="absolute left-3 top-3.5 h-4 w-4 text-slate-400" />
                  <Input value={formData.semester || ""} onChange={(e) => setFormData({...formData, semester: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Current CGPA</label>
                <div className="relative">
                  <GraduationCap className="absolute left-3 top-3.5 h-4 w-4 text-slate-400" />
                  <Input type="number" step="0.01" value={formData.cgpa || ""} onChange={(e) => setFormData({...formData, cgpa: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input value={formData.bloodGroup || ""} onChange={(e) => setFormData({...formData, bloodGroup: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>

              <div className="space-y-2 col-span-full">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Guardian's Contact</label>
                <div className="relative">
                  <Shield className="absolute left-3 top-3.5 h-4 w-4 text-slate-400" />
                  <Input value={formData.guardianPhone || ""} onChange={(e) => setFormData({...formData, guardianPhone: e.target.value})} className="pl-10 h-12" />
                </div>
              </div>
            </div>

            <div className="pt-6 flex justify-end">
              <Button type="submit" disabled={saving} className="bg-blue-600 hover:bg-blue-700 h-14 px-10 rounded-2xl font-black shadow-lg shadow-blue-600/30">
