    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b pb-8">
        <div>
          <h1 className="text-4xl font-black italic uppercase tracking-tighter text-slate-900 flex items-center gap-3">
            <Bell className="text-blue-600 w-8 h-8" /> Notice <span className="text-blue-600">Board</span>
          </h1>
      ...
      <div className="grid gap-6">
        {filteredNotices.length > 0 ? filteredNotices.map((notice) => (
          <Card key={notice._id} className="group border-none shadow-xl shadow-slate-200/50 rounded-[2rem] overflow-hidden bg-white transition-all hover:scale-[1.01]">
            <div className="flex flex-col md:flex-row">
              ...
              {notice.imageUrl && (
                <div className="relative w-full md:w-64 h-48 md:h-auto overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <Badge className={`${getPriorityColor(notice.priority)} px-3 py-1 rounded-lg font-black italic uppercase text-[10px] border shadow-sm`}>
                      {notice.priority}
                    </Badge>
                  </div>
                </div>
              ...
              <CardContent className="p-8 flex-1">
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <Badge variant="outline" className="rounded-full px-3 py-1 text-[9px] font-black italic uppercase text-slate-400 border-slate-200">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase leading-none">Posted By</p>
                      <p className="text-xs font-black italic text-slate-700">{notice.postedBy}</p>
                    </div>
                  </div>
          ...
          <div className="py-20 text-center bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-100">
            ...
            <p className="font-black italic text-slate-300 uppercase tracking-widest">No notices found</p>
          </div>
        )}
      </div>
    </div>
